...
3. Взять директорию из корня за основную рабочую - **/opt** (является частью стандартной файловой системы и предназначена для хранения дополнительного программного обеспечения):
   ...
   - Далее склонировать (подсказка: **git clone**) свой форкнутый репозиторий **linux-monitoring** в свою домашнюю директорию, к примеру в **/Downloads** или в **/Documents**:
      - Должна получиться домашняя директория **/Documents/linux-monitoring**
...
9. Проверить запущенный на коленке **Прометей** (Прометеус) на веб-морде (подсказка: проверка должна быть в браузере на порту **:9090**, чтобы понять по какому ip адресу стучаться в **Прометеус**, нужно знать ip адрес интерфейса своей тачки (тачками называют машины и серверы - это синонимы))
10. Затем переместить из домашней директории **/Documents/prometheus-2.53.2.linux-amd64** в рабочую директорию **/opt/linux-monitoring/prometheus** файлы: бинарь **prometheus**, бинарь **promtool**, и директорию **data** (если её нет, то создать её в рабочей) 
12. Далее создать 2 директории и 2 файла внутри домашней директории **/Documents/linux-monitoring/prometheus** (подсказка: **mkdir consoles console_libraries** - создаст 2 директории сразу):
...
18. Далее одной командой выдать права **777** на корневую директорию **/opt** и на директорию **/Documents/linux-monitoring/** и на все файлы внутри (это называется рекурсивно) 
19. Обычно такие права **777** не выдаются (либо выдаются в песочнице), но у нас учебный проект, поэтому имеем это ввиду:
    ...
    - Заодно научимся понимать, что такое **777** и как эти числа образуются (**4+2+1**)
...
14. Затем рекурсивно назначить (изменить) владельца (пользователя) - **prometheus** для директории - **/opt** и **/Documents/linux-monitoring/** и для всех файлов внутри (проверить результат выполнения):
    - Ниже консольное представление в виде списка **ls** домашней директории **/Documents/linux-monitoring/**:

           --права--  -владелец-  -группа-            /prometheus
           drwxrwxrwx prometheus prometheus           ├── bin/                           #           Директория удалена в процессе написания курса
           -rwxrwxrwx prometheus prometheus           │   └── prometheus                 #           Бинарный файл Prometheus / перемещен в /opt, как и promtool
           drwxrwxrwx prometheus prometheus           ├── etc/                           #           Директория удалена в процессе написания курса
           -rwxrwxrwx prometheus prometheus           │   └── prometheus.yml             # Основной конфигурационный файл Prometheus
           drwxrwxrwx prometheus prometheus           ├── data/                          #           Директория для хранения данных, будет наполнена при работе Prometheus / перемещена в процессе написания курса в /opt
