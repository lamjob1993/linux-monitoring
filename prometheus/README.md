...
| **Золотой сигнал** | **Описание** | **Тип метрики** | **Пример метрики** | **Пример запроса** | **Дополнительные комментарии** |
|--------------------|--------------|-----------------|---------------------|---------------------|--------------------------------|
| **Latency** (Латентность) | Время отклика системы | Histogram/Summary | `http_request_duration_seconds` | `histogram_quantile(0.95, rate(http_request_duration_seconds_bucket[5m]))` | Используется для расчета процентилей времени отклика |
| **Traffic** (Трафик) | Объем входящих запросов | Counter | `http_requests_total` | `rate(http_requests_total[5m])` | Показывает общее количество запросов за определенный период |
| **Errors** (Ошибки) | Количество ошибок | Counter | `http_requests_total{status_code=~"5.."}` | `sum(rate(http_requests_total{status_code=~"5.."}[5m]))` | Фильтрует только ошибочные коды ответа (5xx) |
| **Saturation** (Загрузка) | Уровень загруженности ресурсов | Gauge | `node_cpu_seconds_total`, `node_memory_MemTotal` | `100 - (avg by (instance) (rate(node_cpu_seconds_total{mode="idle"}[5m])) * 100)` | Измеряет использование CPU, памяти, дискового пространства |
