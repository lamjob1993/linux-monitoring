...
3. Взять директорию из корня за основную рабочую - **/opt** (является частью стандартной файловой системы и предназначена для хранения дополнительного программного обеспечения):
   ...
   - Далее склонировать (подсказка: **git clone**) репозиторий [linux-monitoring](https://github.com/lamjob1993/linux-monitoring "linux-monitoring") в свою домашнюю директорию, к примеру в **/Downloads** или в **/Documents**:
      - Должна получиться директория **linux-monitoring**
   - Затем скопировать **linux-monitoring** со всем содержимым внутри в основную рабочую директорию **/opt** и проводить все задания до самого последнего пункта в ней
   - После успешных изменений и финальных прогонов заданий ниже - скопировать целиком директорию **linux-monitoring** обратно в домашнюю и запушить изменения на свой удаленный репозиторий
5. Найти и скачать архив **.tar.gz** дистрибутива **Prometheus** для **Linux** самой последней второй (не третьей) версии в склонированную директорию **/opt/linux-monitoring/prometheus/** (подсказка: GitHub, AMD64, bin).
...
9. Далее переходим в новую рабочую директорию - **/opt/linux-monitoring/prometheus/** и постепенно будем наполнять ее нужными файлами из директории - **prometheus-2.53.2.linux-amd64**.
10. Далее создать директории (подсказка: **sudo mkdir -p** /opt/linux-monitoring/prometheus/**{bin,etc,data,consoles,console_libraries}**):
    - **/bin**
    - **/etc**
    - **/data**
    - **/consoles**
    - **/console_libraries**
    - И положить их в конечную директорию - /opt/linux-monitoring/**prometheus/**
