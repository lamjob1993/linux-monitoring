...
### 1. Как работает Prometheus и для чего он нужен
...
### 2. Pull и Push модель в Prometheus
...
8. **Сервис-дискавери (Service Discovery)**  
   Автоматическое обнаружение целей (таргетов) для сбора метрик (Kubernetes, AWS, Consul и др.).
