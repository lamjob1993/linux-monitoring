...
18. Далее одной командой выдать права `777` на весь внутренний состав директории `/opt/linux-monitoring/**prometheus**` (это называется рекурсивно) 
...
14. Затем рекурсивно назначить владельца `prometheus` для рабочей директории `/opt` (не включая саму директорию `/opt`):
