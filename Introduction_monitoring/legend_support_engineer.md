...
Здесь идёт перечисление из раздела номер 3.

Моя основная задача — поддерживать работоспособность всех систем, разрабатывать мониторинг, решать возникающие инциденты, внедрять релизы и следить за качеством работы серверов и приложений.

В нашем отделе работают: 10 человек + 2 руководителя: тимлид (архитектор) и продакт (основной руководитель проекта по сопровождению). 2 DevOps инженера на уровне middle, 1 senior Go-разработчик и 1 middle Go-разработчик, 3 middle инженера по мониторингу и 3 инженера junior по сопровождению.

Если будут спрашивать на собеседовании о каких-то сложных кейсах и задачах, и если вы не знаете как отвечать, то можно ссылаться на команду, на мидлов и сениоров, что сложные задачи решали совместно с ними. 
...
- Мониторинг состояния систем через Prometheus и Grafana
   - Обращаю внимание на анамалии, если есть: просадка трафика, перезапуск подов, проверка бизнесовых ошибок (это те ошибки которые выдают наши приложения, к примеру, на кредитном конвейере), проверка сервисов, написанных на Java/Kotlin)).
   ...
   - Как решали инцидент? Этапы решения?
      ...
      - Сначала решаю инцидент своими силами:
         ...
         - Проверяю API запрос и понимаю, что запрос неверный:
            - Почему неверный? Я сверил с API документацией по нашему приложению битый API запрос по инциденту. И понял, что один из разработчиков совершил синтаксическую ошибку в коде при раскатке (через Teamcity) и адрес API побился
         - Вручную дёргаю за правильную ручку API (из инструкции) из нужного контейнера или через OpenLens (GUI для Kubernetes/K8S - визуализация кластера)
      - Далее если все-таки не помогло, я с этими ошибками иду: или к разработке, или в аварийную конференцию.
         - Собираю всех вместе в одной конфе.
...
- Подготовка и внедрение релизов через CI/CD-пайплайны совместно с администраторами.
   ...
   - Вы писали тот самый конфиг Nginx указанный выше на балансировку x3 Mimir, описывали последовательность шагов в документации ко внедрению и на совместной встрече с администраторами раскатывали этот конфиг через джобу Teamcity/Jenkins (вы даже можете не знать что такое Teamcity, что это инструмент CI/CD, но на собеседовании можете говорить о нём смело, что за раскатку уже отвечали админы). Внимание! На вашем рабочем проекте может быть только один инструмент или это Teamcity или это только Jenkins.
- Взаимодействие с разработчиками для устранения проблем в коде.
   ...
   - Кстати, спросит собеседующий, а на каком языке у вас написан бэк?
      - Отвечаем Java и Kotlin, но в вашем сопровождении приложения Java.
         - 
...
Если на собеседовании вы чувствуете, что вас долбят вопросами по стеку, то отвечайте спокойно, что Terraform и Ansible вы использовали совместно с командой администраторов, писали конфиги, участвовали во внедрениях с помощью этих инструментов. Потому что на данном этапе вы еще не работали с Docker, Terraform и Ansible и я это понимаю. Но в целом методология вполне рабочая и применимая.
