...
## Здесь я сделал специально вбросы от собеседующих буквально: в ваш рассказ о себе, об отделе, о распорялке дня
...
- Мониторинг состояния систем через Prometheus и Grafana:
   ...
   - Куда смотришь в Grafana (на какие имменно дашборды)?
      ...
      - Кто поднимал экспортеры и дашборды?
         - Ответ: Я поднимал через юнит сервисы все экспортеры и рисовал кастомные дашборды на основе официальных с сайта Grafana Lab. Далее здесь в разделе на примере Nginx будет рассказано, как поднимается сервис (для собеседования). В целом после прохождения этого репозитория у вас будут эти вопросы отскакивать как от зубов.
...
- В процессе вашего рассказа по распорядку вам могут задать пару вопросов:
