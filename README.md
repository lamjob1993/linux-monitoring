...
26. Далее написать **Bash** скрипт, начиная с пункта скачивания **Prometheus** по ссылке до полноценного старта сервиса **Prometheus**:
    ...
    - Писать эти скрипты строго используя логические операторы **&&** и **||**:
       - То есть каждая строка скрипта должна проверяться оператором && - на **успех операции** и в противном случае || - **команда не отработана**

         # Удаление директории && вывод сообщения об успехе || вывод сообщения об ошибке
