...
- Последовательность выполнения заданий следующая (читаем в разделах файл **readme.md** и исполняем):
  1. Настройте доступ по SSH по инструкции выше
  2. Сделайте форк по инструкции выше, сделайте клон репозитория себе на локальный ПК, создайте любой текстовый файл и сделайте первый коммит и пуш в ваш клон
  3. Открываем директорию `prometheus` -> `README.md` -> `Backend.md` -> `config-file.md`
  4. Открываем директорию `grafana` -> `README.md`
  5. Открываем директорию `process-exporter` -> `README.md`
  6. Открываекм директорию `blackbox-exporter` -> `README.md`
  7. Открываем директорию `node-exporter` -> `README.md`
  8. После исполнения полностью настройте бэкенд мониторинга: натравите `Prometheus` на все экспортеры и замониторьте ваш сервер [ваша тачка] в `Grafana`
