...
1. Изначально у вас должна быть установлена виртуалка Ubuntu 22.04:
   - После установки склонируйте её и продолжайте работать на клоне (это нужно в целях резервирования и для дальнейших заданий)
...
7. Проверить запущенный на коленке **Прометей** (Прометеус) на веб-морде (подсказка: проверка должна быть в браузере на порту **:9090**, чтобы понять по какому ip адресу стучаться в **Прометеус**, нужно знать ip адрес интерфейса своей тачки).
...
11. Скопировать в **/bin** бинарь **Prometheus** (подсказка: **п.4**).
12. В директории **/etc** создать пустой файл - **prometheus.yml**, затем наполнить **prometheus.yml** дефолтным конфигом **Prometheus** (подсказка: **GitHub** - **prometheus/documentation/examples/prometheus.yml**, а также подумать, что означает этот конфиг построчно на пару с **Qwen GPT**).
13. Наполнить директории: **/data** - оставить пустой, **/consoles** - наполнить по аналогии с директорией - **prometheus-2.53.2.linux-amd64**, **/console_libraries** - наполнить по аналогии с директорией - **prometheus-2.53.2.linux-amd64**.
14. Далее создать пустой файл - **Unit** в корневой директории - **/prometheus** и наполнить (на сленге - написать) его для **Prometheus** (подсказка: логически понять, как назвать **Unit** файл (это будет файл сервиса **Prometheus**) и как он должен быть написан внутри).
15. Далее одной командой выдать права **777** на корневую директорию **/opt** и на все файлы внутри (это называется рекурсивно), в которой уже должны лежать файлы по структуре ниже (проверить результат выполнения команды): 
...
16. Обычно такие права **777** не выдаются (либо выдаются в песочнице), но у нас учебный проект, поэтому имеем это ввиду:
...
12. Далее нужно создать группу пользователей **prometheus** и добавить туда пользователя **prometheus**:
    ...
    - Здесь мы делаем назначение вручную, чтобы понимать логику работу команд: **chown** и **chmod**
...
15. Затем скопировать **Unit** в директорию **/etc/systemd/system/** автозапуска Linux с уже выставленными правами **777** и владельцем **prometheus**.
16. Чтобы система Linux поняла, что файл сервиса добавлен в автозапуск, нужно вбить команду - **sudo systemctl daemon-reload**:
...
24. Далее, так как мы подняли **Prometheus** и он работает — пробежимся по дебагу (просто представим, что приложению нужен дебаг, а-ля это инцидент в банке и его нужно решать, а **Прометей** у нас не работает):
...
25. Далее написать **Bash** скрипт, начиная с пункта 2 текущей темы до старта сервиса Prometheus, включая написанный **Unit** и конфиг **Prometheus** в директории **/bin**. Этот скрипт в финале должен поднять **Prometheus** с нуля за секунды и вы должны увидеть его на веб-морде (придумать в скрипте свои директории, к примеру **test_prometheus_2** (а-ля свой проект, к примеру **Prometheus_2**) и сложить файлы **Prometheus** туда).
26. Прогнать готовый скрипт (не забыть удалить юнит из автозапуска и стопнуть поднятый Prometheus, который уже поднимали).
27. На данном этапе сохранить ваши текущие изменения и выключить виртуалку.
28. Далее перейти в другую виртуальную машину **Ubuntu**, которая осталась рядом с клоном из **п.1** и запустить её рядом с вашим клоном **Ubuntu** одновременно.
29. Подумать, как установить между этими двумя тачками **А** и **Б** соединение **SSH**, чтобы можно было ходить из одной машины на другую (подсказка: потренироваться генерить **SSH ключи**). 
30. Далее подумать, как с помощью утилиты **SCP** перекинуть рабочий и подготовленный архив **Prometheus** с тачки **А** на тачку **Б** (подумайте, как запаковать его уже с выданными правами и конфигами, **Prometheus** должен быть готов к запуску по вашему скрипту на новой тачке (тачками называют машины и серверы - это синонимы)).
31. Финал. Прогнать готовый скрипт на виртуалке.
