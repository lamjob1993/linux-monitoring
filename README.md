...
### Последовательность выполнения заданий
  ...
  16. Открываем директорию **`interview_questions`** → `1` → `2` → `3`
  17. После исполнения всех пунктов выше (**добавить раздел, как считать количество и скорость метрик для каждой команды, снять метрики с Mimir и посчитать единицу измерения на рубль в отдельном дашборде**):
  ...
  18. Далее нужно будет создать от своего лица публичный репозиторий в **GitHub** на тему мониторинга, который будет служить для вас, как портфолио для работодателя (так называемый [Pet-проект](https://practicum.yandex.ru/blog/chto-takoe-pet-proekty-idei-dlya-novichkov/ "С помощью пет-проектов можно улучшить навыки и найти работу. Рассказываем, как их создавать, чем вдохновляться, а также какие у таких проектов есть плюсы и минусы."))
  ...
  19. Продолжаем изучение мониторинга в [следующем разделе](https://github.com/lamjob1993/docker-monitoring/tree/main) по **`Docker`**
  20. Продолжаем изучение мониторинга в [следующем разделе]() по **`Terraform`**
  21. Продолжаем изучение мониторинга в [следующем разделе]() по **`Ansible`**
  22. Продолжаем изучение мониторинга в [следующем разделе]() по **`CI/CD: Teamcity`**
  23. Продолжаем изучение мониторинга в [следующем разделе]() по **`Kubernetes`**
