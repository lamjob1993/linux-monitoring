...
_Задание написано таким образом, что о тех инструментах, которые в нем используются, спрашивают более чем в 80% случаев на собеседовании по части Linux у джунов: **DevOps, SRE, инженеров по сопровождению/мониторингу и администраторов Linux**. Задание носит более лабораторный характер, чем полезный, и я это понимаю, но оно позволит наработать нужную практику. Сразу всё задание со всеми пунктами через ChatGPT (Qwen) не прогоняем, иначе не пройдем собеседование (пользуемся только Qwen по точечным запросам, к примеру, как работает команда на выдачу прав, потому что это сильно быстрее, чем поиск в Google). Делаем пункты строго по очереди, не перепрыгиваем!_
...
1. Изначально у вас должна быть установлена первичная виртуалка **Ubuntu 22.04**:
...
22. Далее парсим (фильтруем) логи утилитой **grep** в файл **logs.log** по ключевому слову **info**:
    ...
    - А также подумайте, как с помощью **grep**:
       - Добавить новые записи в файл с сохранением в реальном времени (то есть не через однократный вывод) 
       - Сделать мини-скрипт, который будет грепать новые логи раз в минуту, записывая в файл:
          - Поставить скрипт на планировщик **crontab** на выполнение
...
26. Далее написать **Bash** скрипт, начиная с пункта скачивания **Prometheus** по ссылке до полноценного старта сервиса **Prometheus**:
    - Этот скрипт в финале должен поднять **Prometheus** с нуля за секунды и вы должны увидеть в терминале статус сервиса **ACTIVE**, а затем пойти и увидеть его на веб-морде
    ...
    - Оставить эти два скрипта нетронутыми
30. Далее перейти в первичную виртуальную машину **Ubuntu**, которая осталась рядом с клоном из **п.1** и запустить её рядом с вашим клоном **Ubuntu** одновременно.
31. Прогнать из пункта выше готовый скрипт запуска **Prometheus** на первичной (голой) машине:
...
33. Подумать, как установить между этими двумя тачками **А** и **Б** соединение **SSH**, чтобы можно было ходить из одной машины на другую:
    - Подсказка: потренироваться генерить **SSH ключи** и понять, что такое публичный и закрытый ключи
34. Далее модернизировать старый **Bash** скрипт, который будет архивировать с нуля уже подготовленный **Prometheus** со всеми файлами (можно взять старые файлы из-под запущенного **Prometheus**):
    - Нужно написать скрипт таким образом, чтобы при распаковке на новой тачке автоматом выставлялись права и нужный пользователь на директорию **/opt** рекурсивно (тут важно помнить, что пользователя и группы пользователей **Prometheus** на голой машине ещё не существует)
    - **Prometheus** должен быть готов к запуску по вашему скрипту на новой тачке
36. Далее подумать, как с помощью утилиты **SCP** перекинуть этот архив **Prometheus** с тачки **А** (клон) на тачку **Б** (первичная):
    - Прогнать готовый скрипт на виртуалке
