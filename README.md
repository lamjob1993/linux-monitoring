...
- Последовательность выполнения заданий следующая (читаем в разделах файл **readme.md** и исполняем):
  1. Открываем директорию `prometheus` -> `README.md` -> `Backend.md` -> `config-file.md`
  2. Открываем директорию `grafana` -> `README.md`
  3. Открываем директорию `process-exporter` -> `README.md`
  4. Открываекм директорию `blackbox-exporter` -> `README.md`
  5. Открываем директорию `node-exporter` -> `README.md`
