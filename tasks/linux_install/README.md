...
- Для установки берем чистый образ [Debian Netinst 13.5.0 (amd64)+](https://www.debian.org/CD/netinst/ "Это один компакт-диск, который содержит только самое необходимое программное обеспечение для того, чтобы установить базовую систему и загрузить все остальные пакеты по сети.") и ставим с [Desktop Environment (DE)](https://ru.wikipedia.org/wiki/%D0%A1%D1%80%D0%B5%D0%B4%D0%B0_%D1%80%D0%B0%D0%B1%D0%BE%D1%87%D0%B5%D0%B3%D0%BE_%D1%81%D1%82%D0%BE%D0%BB%D0%B0 "Среда рабочего стола (СРС, англ. desktop environment) — разновидность графических интерфейсов пользователя, основанная на метафоре рабочего стола.")
- Для **VMWare Workstation Pro** выбираем следующие параметры:
  - **RAM**: `2048Mb`
  - **SSD**: `30Gb` / можно не бояться за объем, он будет расти по мере потребления, после установки занято будет где-то 10ГБ, но расширять в дальнейшем сильно сложнее, проще взять универсально 30
  - **CPU**: `4 Cores`
  - **Network**: `NAT`
...
```
- Установка пакета автодополнения слов по клавише tab
```bash
...
- Клонируем 2 раза основную виртуальную машину Debian со следующими настройками и получаем 3 машины
...
- С данного этапа на ваши машины Debian вы будете заходить только из-под VmWare и только в фоновом режиме (клик правой кнопкой на запуск в фоне) и только через **MobaXterm**
...
- Попробуйте настроить ssh плагин VS Code до вашей основной Debian тачки (когда пойдут длинные Docker Compose - это будет необходимо)
