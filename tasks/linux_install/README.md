...
- Это связано с тем, что в Debian не достает достаточного количества настроек, пакетов, пользователей и т.д, иными словами говоря на примере студентов я понял, что это головная боль, а когда вы будете работать на ПРОДе в рабочей среде, то каждая машина уже либо будет настроена (сопровождение/мониторинг)
